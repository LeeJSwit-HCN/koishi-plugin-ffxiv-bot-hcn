...
var schema = import_koishi.Schema.intersect([
  ...
  import_koishi.Schema.object({
    ...
    IsSell: import_koishi.Schema.boolean().default(true).description("删除可能已经卖掉的物品价格"),
    ToBuy: import_koishi.Schema.boolean().default(true).description("生成推荐去哪个服务器买东西")
  }).description("查价设置"),
  import_koishi.Schema.object({
    EntriesToReturn: import_koishi.Schema.number().role("slider").min(5).max(100).step(1).default(12).description("查历史结果行数")
  }).description("查历史设置")
]);
function apply(ctx, config) {
  ctx.command("查价").alias("/查价").alias("cj").option("server", "-s <server> 目标服务器", { fallback: config.DataCenter.Server }).option("limit", "-l <limit> 结果行数", { fallback: config.Limit }).option("gst", "-g 输出税后价格", { fallback: config.Gst }).option("isSell", "-i 删除可能已经卖掉的物品", { fallback: config.IsSell }).option("toBuy", "-b 生成推荐去哪个服务器买", { fallback: config.ToBuy }).example("查价 巨匠药酒 -s 猫小胖 -l 20 -g 解释：查询猫小胖服巨匠药酒20条税后价格数据").action(({ session, options }, input) => {
    if (!input?.trim()) {
...
__name(apply, "apply");
async function getPrices(ctx, session, itemId, itemName, options) {
  ...
  if (universalis_recvJson.listings.length != 0) {
    if (options.isSell) {
      ...
      itemList = itemList.filter((obj) => {
        return Object.values(obj).every((value) => value !== null && value !== void 0 && value !== "");
      });
...
async function getHistory(ctx, session, itemId, itemName, options) {
  let server = options.server;
...
function isSell(pricesList, hisList) {
  return pricesList.filter(
    (priceItem) => !hisList.some(
      (histItem) => priceItem.worldName === histItem.worldName && priceItem.pricePerUnit === histItem.pricePerUnit && priceItem.quantity === histItem.quantity && priceItem.hq === histItem.hq
    )
  );
}
